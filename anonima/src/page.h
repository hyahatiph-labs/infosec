...
#define TMPL_SWAP                   TMPL_DIR "/swap.html"
#define TMPL_SWAP_RESULTS           TMPL_DIR "/swap_results.html"
...
// them only once, when the board starts into this map
// this will improve performance of the board and reduce
          ...
          mainnet_url {_mainnet_url}
{
    ...
    template_file["swap"]            = get_full_page(xmreg::read(TMPL_SWAP));
    template_file["swap_results"]    = get_full_page(xmreg::read(TMPL_SWAP_RESULTS));
    ...
    else
    {
        ...
        if (this->enable_pusher == false)
        {
            string error_msg = fmt::format(
                    "Pushing disabled!\n "
                            "Run board with --enable-pusher flag to enable it.");
...
// swap client interface
// TODO: refresh seller list in the background, asynchronously
const int INITIAL_STATUS_DELAY = 30000;
string seller_list;

void
set_seller_list(string s)
{
    seller_list = s;
}
...
// helper function for accessing the swap binary
string
execute_swap_cmd(string cmd)
{
...
void
bg_execute_swap_cmd(string cmd)
{
    unique_ptr<FILE, decltype(&pclose)> pipe(popen(cmd.c_str(), "r"), pclose);
    if (!pipe) {
        throw std::runtime_error("popen() failed!");
...
start_swap()
{
    string full_page = template_file["swap"];
    ...
    add_css_style(context);
    if (this->enable_swaps == false)
        {
            string error_msg = fmt::format(
                    "Swaps disabled!\n "
                            "Run board with --enable-swaps flag to enable it.");
    ...
    // parse logs
    context.emplace("sellers" , mstch::array());
    // get reference to blocks template map to be field below
    mstch::array& sellers = boost::get<mstch::array>(context["sellers"]);
    std::vector<std::string> list_seller_logs;
    // TODO: commands based on stagenet or mainnet mode
    const string list_seller_cmd = this->stagenet
        ? fmt::format("stdbuf -o 0 ./swap --testnet --json list-sellers " 
    "--rendezvous-point {:s}", rendezvous_point)
        : fmt::format("stdbuf -o 0 ./swap --json list-sellers " 
    "--rendezvous-point {:s}", rendezvous_point);
    set_seller_list(execute_swap_cmd(list_seller_cmd));
    boost::split(list_seller_logs, get_seller_list(), boost::is_any_of("\n"));
    // iterate through log data and create a array of sellers
    for (auto seller : list_seller_logs) {
        if (seller != "") {
            json j_seller = json::parse(seller);
            if (j_seller.count("status") > 0 && j_seller["status"].count("Online") > 0) {
                string price = to_string(j_seller["status"]["Online"]["price"].get<int>());
                string min = to_string(j_seller["status"]["Online"]["min_quantity"].get<int>());
                string max = to_string(j_seller["status"]["Online"]["max_quantity"].get<int>());
                std::vector<std::string> v_peer;
                boost::split(v_peer, j_seller.value("multiaddr", ""), boost::is_any_of("/"));
                sellers.push_back(mstch::map {
                    {"id"       , v_peer[4]},
                    {"price"    , price},
                    {"min"      , min},
                    {"max"      , max}
...
string
show_status(string btc_address, string xmr_address, string peer_id)
{
    string full_page = template_file["search_results"];
    // initalize page template error context map
    mstch::map context {
            {"has_error"           , false},
            {"error_msg"           , string {}},
    };
    ...
    string client_id = fmt::format("{:s}{:d}", xmr_address, std::time(nullptr));
    string inject_seller;
    std::vector<std::string> list_seller_logs;
    boost::split(list_seller_logs, get_seller_list(), boost::is_any_of("\n"));
    for (auto seller : list_seller_logs) {
        if (seller != "") {
            json j_seller = json::parse(seller);
            std::vector<std::string> v_peer;
            string seller_id = j_seller.value("multiaddr", "");
            boost::split(v_peer, seller_id, boost::is_any_of("/"));
            if (v_peer[4] == peer_id) {
                inject_seller = seller_id;
            }
        }
    }
    // use combo of timestamp and rx address to create client id for session management
    // once swap is started user should be able to get swap_id from status for history, resume, cancel etc.
    string cmd = this->stagenet ? fmt::format("stdbuf -o 0 ./swap --testnet --json buy-xmr " 
        "--change-address {:s} --receive-address {:s} --seller {:s} > /tmp/swap{:s}.log 2>&1 &",
        btc_address, xmr_address, inject_seller, client_id)
        : fmt::format("stdbuf -o 0 ./swap --json buy-xmr " 
        "--change-address {:s} --receive-address {:s} --seller {:s} > /tmp/swap{:s}.log 2>&1 &",
        btc_address, xmr_address, inject_seller, client_id);
    // will be running this in the background and using client_id to monitor
    bg_execute_swap_cmd(cmd);
    // render the page, TODO: testing latency to read the initial status
    std::this_thread::sleep_for(std::chrono::milliseconds(INITIAL_STATUS_DELAY));
    return get_swap_status(client_id, false);
}

string
get_swap_status(string client_id, bool refresh)
{
    string full_page = template_file["swap_results"];;
    // initalize page template context map
    mstch::map context {
            {"client_id"            , client_id},
            {"refresh"              , refresh},
            {"depsit_address"       , string {}},
            {"message"              , string {}},
            {"has_error"           , false},
            {"error_msg"           , string {}}
    };
    ...
    string cmd = fmt::format("stdbuf -o 0 tail -1 /tmp/swap{:s}.log ", client_id);
    context["client_id"] = client_id;
    json j_status = json::parse(execute_swap_cmd(cmd));
    if (j_status.count("fields") > 0 && j_status != "") {
        json j_fields = j_status["fields"];
        string deposit_address = j_fields.count("deposit_address") > 0 ? j_fields["deposit_address"] : "";
        string message = j_status["fields"]["message"];
        // TODO: parse swap_id
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    // are spent, and unknown umounts (makrked in board by '?') when
